...
def searchInit(request):
    ...
    return render(request, 'Forecast/SerachHistory.html', {'result_code':0,'data_dict': data_dict,'image_url':data_dict[files[0]]})
...
def searchHistory(request):
    ...
    if request.method == 'POST':
        ...
        if (request.POST['area'] == '全球' and request.POST['factor'] == 'F0'):
            dir_path = os.path.join(root_path,
                                    settings.AREA_DICT[request.POST['area']],
                                    settings.FACTOR_DICT[request.POST['factor']])
            ...
            for filename, filepath in data_dict.items():
                ...
                html += line
            return_json = {'result_code': 0, 'image_url': data_dict[files[0]], 'html': html}

        elif (request.POST['area'] == '全球' and request.POST['factor'] == 'F5'):
            dir_path = os.path.join(root_path,
                                    settings.AREA_DICT[request.POST['area']],
                                    settings.FACTOR_DICT[request.POST['factor']])
            data_dict, files = iterator_dir(dir_path)
        ...
        elif (request.POST['area'] == '西北太' and request.POST['factor'] == 'F6'):
            dir_path = os.path.join(root_path,
                                    settings.AREA_DICT[request.POST['area']],
                                    settings.FACTOR_DICT[request.POST['factor']])
            data_dict, files = iterator_dir(dir_path)
        ...
        elif (request.POST['area'] == '北印度洋' and request.POST['factor'] == 'F6'):
            dir_path = os.path.join(root_path,
                                    settings.AREA_DICT[request.POST['area']],
                                    settings.FACTOR_DICT[request.POST['factor']])
            data_dict, files = iterator_dir(dir_path)
            ...
            return_json = {'result_code': 1, 'image_url': '', 'html': html}
